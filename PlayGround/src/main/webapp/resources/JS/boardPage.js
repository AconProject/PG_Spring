...
/* 페이징함수 호출함수 */
function prepareForPaging(replies, replyLiked) {
	let totalData = replies.length; // 총 댓글 수
	let maxDataPerPage = 5; // 한 페이지에 나타낼수 있는 댓글 수
	let maxPagePerWindow = 5; // 한 화면에 나타낼수 있는 페이지 수
	paging(replies, replyLiked, totalData, maxDataPerPage, maxPagePerWindow, 1);
}

/* 페이징 처리 */
function paging(replies, replyLiked, totalData, maxDataPerPage, maxPagePerWindow, currentPage) {
	...
	jsonParserForBoardReply(replies, replyLiked, start, end);
	...
	pages.forEach(page => {
		page.addEventListener('click', function () {
			...
			paging(replies, replyLiked, totalData, maxDataPerPage, maxPagePerWindow, selectedPage);
		}, false);
	});
}
...
/* 게시글 댓글 파싱 후 출력 */
function jsonParserForBoardReply(replies, replyLiked, start, end) {
	...
	for (let i = start; i < replies.length && i < end; i++) {
		...
		html +=
			...
			'<input type="hidden" id="isLiked' + replies[i].replyId + '" value="' + replyLiked[i] + '">';
		...
		.then(replies => {
			if (replies.length !== 0) {
				fetch('../../reply/replies/boards/' + boardUrlId)
					.then(res => res.json())
					.then(replyLiked => {
						prepareForPaging(replies, replyLiked);
					})
					.catch(err => {
						console.log(err);
					});
